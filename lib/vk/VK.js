...
class VK extends Module {
        ...
        this.bot.event("message_new", async (ctx) => {
            this.#ModuleManager.workMessage(
                ...
                new Response(
                    ...
                    async (photo) => {await this.sendPhotosByURL(this,ctx,photo.url)}
                    )
            );
        });
    ...
    async getUser(module = this,ctx) {
        let user = (await module.bot.execute('users.get',{user_id:ctx.message.from_id,fields:"domain"}))[0];
    ...
    async uploadPhoto (module,filePath, peerId) {
        const { upload_url: url } = await module.bot.execute('photos.getMessagesUploadServer', {
            peer_id: peerId,
        })
        ...
        const [photo] = await module.bot.execute('photos.saveMessagesPhoto', data)
